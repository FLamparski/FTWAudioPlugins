...
def  main():
  ...
  installer = "\installer\WeirdOrgan Installer.exe"
   
  if demo:
    installer = "\installer\WeirdOrgan Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\WeirdOrgan manual.pdf" 
  ]
  ...
  ZIPNAME = "WeirdOrgan-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "WeirdOrgan-v" + FULLVERSIONSTR + "-win-demo.zip"
